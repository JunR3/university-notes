...
The central idea of ResNet is simple: instead of asking a stack of layers to learn a mapping \emph{from scratch}, give the input a direct path to the output (the \term{skip connection} or \term{shortcut}), and ask the layers to learn only what needs to \emph{change} — the \emph{residual}.

Concretely, suppose a block receives input $\mathbf{x}$ and we want it to compute some desired mapping $\mathcal{H}(\mathbf{x})$. Rather than having the conv layers approximate $\mathcal{H}(\mathbf{x})$ directly, we rewrite the target as:
\[
  \mathcal{H}(\mathbf{x}) = \underbrace{\mathcal{F}(\mathbf{x})}_{\text{learned by conv layers}} + \underbrace{\mathbf{x}}_{\text{identity shortcut}}
\]
where $\mathcal{F}(\mathbf{x}) \coloneqq \mathcal{H}(\mathbf{x}) - \mathbf{x}$ is the \term{residual} — the difference between the desired output and the input. The skip connection adds $\mathbf{x}$ directly to the output of the conv layers \emph{without} passing through any weights.

\medskip
\textbf{Why is this easier to optimise?} If the block should ideally do nothing (i.e.\ $\mathcal{H}(\mathbf{x}) = \mathbf{x}$), the conv layers only need to learn $\mathcal{F}(\mathbf{x}) = 0$ — drive all weights to zero. Pushing weights toward zero is much easier than pushing them to reproduce an arbitrary identity mapping. In practice, most blocks in a trained ResNet learn small residuals, meaning each block makes only a minor correction to the representation rather than a wholesale transformation.

\begin{defbox}{Residual Block}
A residual block consists of two (or three) conv layers arranged so that the input $\mathbf{x}$ bypasses them via a skip connection:
\[
  \text{output} = \sigma\!\bigl(\mathcal{F}(\mathbf{x}) + \mathbf{x}\bigr)
\]
where:
\begin{itemize}
  \item $\mathcal{F}(\mathbf{x})$ is the output of the stacked conv layers (two $3\times3$ convs in the basic block), with no final activation before the addition.
  \item $\mathbf{x}$ is the \term{identity shortcut}: the raw input added element-wise to $\mathcal{F}(\mathbf{x})$.
  \item $\sigma$ is a ReLU applied \emph{after} the addition — not between the two conv layers and the shortcut.
\end{itemize}

\medskip
\textbf{Shape constraint.} The element-wise addition requires $\mathcal{F}(\mathbf{x})$ and $\mathbf{x}$ to have exactly the same shape (same height, width, and number of channels). This is automatically satisfied as long as the block does not change spatial size or channel count. If either does change (e.g.\ when transitioning between stages), a \emph{projection shortcut} — a $1\times1$ conv applied to $\mathbf{x}$ — is used to match dimensions before the addition.
\end{defbox}
...
The conceptual link is this: CAM and Grad-CAM are both performing the same operation — weighting spatial feature maps by their importance to a class and summing — but CAM reads the importance weights from the trained FC layer, while Grad-CAM \emph{computes} them on-the-fly using gradients. This is why Grad-CAM reduces exactly to CAM when the architecture has GAP + single FC (the gradient average equals the FC weight in that special case).
...
\begin{enumerate}
  ...
  \item \textbf{ROI Pooling}: for each projected region of interest, extract a fixed-size $H \times W$ activation by dividing the ROI into an $H \times W$ grid and applying max-pooling within each cell. This yields a fixed-length vector regardless of the original ROI size.
...
An alternative family of methods replaces the discrete count-based approach with \emph{continuous} representations of words. These include:
\begin{itemize}
  \item \textbf{Latent Semantic Analysis (LSA)}: SVD-based factorisation of word--document co-occurrence matrices.
  \item \textbf{Latent Dirichlet Allocation (LDA)}: probabilistic topic modelling.
  \item \textbf{Distributed representations} (neural embeddings): the approach we focus on, pioneered by Bengio et al.\ (2003) and dramatically scaled by Mikolov et al.\ (2013).
\end{itemize}
